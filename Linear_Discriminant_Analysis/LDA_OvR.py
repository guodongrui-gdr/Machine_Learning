...
for label in range(10):
    ...
    # 数据标准化
    scaler = preprocessing.StandardScaler().fit(np.concatenate((x0, x1),
    ...
    # 训练模型
    w, w0 = train(x0, x1)
    classifier.append((w, w0))  # 将训练好的模型参数存入数组中
# 测试模型
label = 0  # 选取一类数字作为测试对象
test_x = data[target == label, :]
result = []
for i in range(10):
    w = classifier[i][0]
    w0 = classifier[i][1]
...
result.sort(key=lambda x: (x[1]), reverse=True)  # 将十个分类器得出的结果按准确率降序排序
print("classify result is:", result[0][0])  # 最终结果为准确率最高的分类器对应的数字
